/**
 * schema.ts — DB init
 * babel.config.js aliases 'expo-sqlite' → src/mocks/expo-sqlite for Expo Go.
 * We import via the mock path directly so TypeScript never touches the real
 * expo-sqlite types (which differ between SDK versions).
 */

// Use the mock directly so TS resolves our own types, not expo-sqlite's
import { openDatabase } from '../mocks/expo-sqlite';

export const db = openDatabase('bromood.db');
    ...
    db.transaction(
      (tx: any) => {
        const statements: [string, any[]][] = [
          ...
          // Use ? args so the mock INSERT correctly maps values

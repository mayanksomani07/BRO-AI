/**
 * Database queries — typed helpers for all DB operations
 */
...
export async function insertJournalEntry(data: Omit<JournalEntry, 'id'>): Promise<string> {
  ...
  await db.runAsync(
    `INSERT INTO journal_entries (id, content_encrypted, sentiment, detected_themes, mood_score_at_time, created_at)
     VALUES (?, ?, ?, ?, ?, ?)`,
    [id, data.content_encrypted, data.sentiment, data.detected_themes,
     data.mood_score_at_time, data.created_at]
  );
  return id;
}
...
export async function getChatHistory(limit = 100): Promise<ChatMessage[]> {
  return db.getAllAsync<ChatMessage>(
    `SELECT * FROM chat_messages ORDER BY sent_at ASC LIMIT ?`,
    [limit]
  );
}
...
export async function markTaskCompleted(taskId: string, xp: number): Promise<void> {
  const id = Crypto.randomUUID();
  await db.runAsync(
    `INSERT OR REPLACE INTO task_progress (id, task_id, completed, xp_earned, completed_at)
     VALUES (?, ?, 1, ?, ?)`,
    [id, taskId, xp, Date.now()]
  );
  await db.runAsync(
    `UPDATE user_stats SET total_xp = total_xp + ?, last_task_completed_at = ? WHERE id = 'singleton'`,
    [xp, Date.now()]
  );
...
async function updateUserLevel(): Promise<void> {
  ...
  await db.runAsync(
    `UPDATE user_stats SET current_level = ? WHERE id = 'singleton'`,
    [level]
  );
}

export async function getUserStats(): Promise<UserStats | null> {
  const rows = await db.getAllAsync<UserStats>(
...
export async function wipeAllData(): Promise<void> {
  await db.execAsync(`
    ...
    UPDATE user_stats SET total_xp = 0, current_level = 'Naya Bro', streak_days = 0, badges = '[]';
  `);
}
...
export async function exportAllData(): Promise<object> {
  const [moods, journal, chat, tasks, stats] = await Promise.all([
    ...
    getChatHistory(500),
    db.getAllAsync('SELECT * FROM task_progress'),
    getUserStats(),
  ]);
